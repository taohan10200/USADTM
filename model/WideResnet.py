...
class WideResnet(nn.Module):
    ...
    '''
    def __init__(self, n_classes, k=2, n=28, batchsize=64):
        super(WideResnet, self).__init__()
        self.n_layers, self.k = n, k
        self.batchsize = batchsize
        self.backbone = WideResnetBackbone(k=k, n=n, drop_rate =0)
    ...
    def forward(self, x, mode='train'):
        ...
        f = torch.mean(feat, dim=(2, 3))

        if mode=='train':
            f_lbeled, f_unlabeled  = f[:self.batchsize],f[self.batchsize:]
            pred_l = self.classifier(f_lbeled)
            pred_u = self.classifier(f_unlabeled)

            return f_lbeled, f_unlabeled, pred_l, pred_u
        else:
